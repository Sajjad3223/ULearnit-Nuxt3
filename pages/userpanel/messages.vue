  ...
  <div>
    ...
    <div className="w-full mt-4">
      <u-alert v-for="n in notifs" :color="`${n.isSeen ? 'dark' : 'primary'}`">
        <template #icon>
          <svg v-if="n.icon === EIcon.Course" class="flex-shrink-0 inline w-5 h-5 ml-3" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <span class="font-medium" v-if="n.icon === EIcon.Course">اطلاع رسانی قسمت جدید!</span>
        <NuxtLink @click="seenNotif(n.id,n.link)" class="font-bold mr-2 cursor-pointer">{{n.title}}</NuxtLink>
