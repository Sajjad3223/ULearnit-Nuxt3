<template>
  <div>
    ...
    <u-modal title="افزودن تیکت جدید" v-model="ticketModal">
      <template #default>
        <Form @submit.prevent="createTicket" class="flex flex-col p-4 space-y-4">
          <base-input
              name="title"
              label="عنوان"
              v-model="addTicketData.title"
          />
          <base-input multiline
              name="content"
              label="متن تیکت"
              v-model="addTicketData.text"
          />
          <base-button type="submit">ثبت تیکت</base-button>
        </Form>
      </template>
    </u-modal>
    ...
    <div class="w-full mt-4">
      <u-table v-if="tickets !== undefined && tickets.length > 0" :pagination-data="paginationData" >
        ...
        <template #table-body="{showOption}">
          <tr class="border-b dark:border-gray-700" v-for="(t,i) in tickets">
            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
              <NuxtLink :to="`/userpanel/tickets/${t.id}`" class="link" >{{t.title}}</NuxtLink>
            </th>
            <td>{{ new Date(t.creationDate).toLocaleDateString('fa-IR') }}</td>
            <td class="px-4 py-3">
              <u-badge v-if="t.ticketStatus === 0">در حال بررسی</u-badge>
              <u-badge color="success" v-if="t.ticketStatus === 1">پاسخ داده شده</u-badge>
              <u-badge color="dark" v-if="t.ticketStatus === 2">بسته شده</u-badge>
            </td>
