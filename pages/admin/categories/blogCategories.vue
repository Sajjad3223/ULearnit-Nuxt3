<template>
  <div>
    <u-divider title="دسته بندی های مقالات"/>

    <admin-categories-create-modal v-model="categoryModal" @categoryCreated="onCategoryCreated" category-type="blog"/>
    <admin-categories-add-child-modal ref="childCategoryModal" v-model="childCategoryModalModel" @childCategoryCreated="onCategoryCreated" category-type="blog"/>
    ...
    <u-table ref="dataTable">
      <template v-slot:table-options="{showFilter}">
        <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 md:space-x-reverse flex-shrink-0">
          <button @click="categoryModal = true"  class="flex items-center justify-center text-white bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:ring-indigo-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-indigo-600 dark:hover:bg-indigo-700 focus:outline-none dark:focus:ring-indigo-800">
      ...
      <template #table-header>
        ...
        <th scope="col" class="px-4 py-3">عنوان دسته بندی</th>
      ...
      <template v-slot:table-body="{showOption}">
        <admin-categories-category-row v-for="(c,i) in categories"
                                       :key="c.id"
                                       :category="c"
                                       :index="i"
...
const childCategoryModal = ref();
const categories = ref();
const dataTable = ref();

const onCategoryCreated= async ()=>{
  categoryModal.value = false;
  childCategoryModalModel.value = false;
  await loadData();
}
...
const addChild=(category:CategoryDto)=>{
  childCategoryModal.value.setParentId(category.id);
  childCategoryModalModel.value = true;
