    ...
    <u-table ref="dataTable" :pagination-data="paginationData">
      ...
      <template v-slot:table-body="{showOption}">
        <tr class="border-b dark:border-gray-700" v-for="(r,i) in requests">
          ...
          <td class="px-4 py-3">
            <u-badge v-if="r.status === EMasterStatus.Active" color="success">تایید شده</u-badge>
            <u-badge v-if="r.status === EMasterStatus.Inactive" color="dark">رد شده</u-badge>
            <u-badge v-if="r.status === EMasterStatus.Pending">در حال بررسی</u-badge>
          </td>
          <td class="px-4 py-3 flex items-center justify-end">
            ...
            <div class="table-option absolute hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
              <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="apple-imac-27-dropdown-button">
                <li>
                  <button v-if="r.status === EMasterStatus.Pending || r.status === EMasterStatus.Inactive" @click.prevent="accept(r.id)" class="w-full text-right block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">قبول درخواست</button>
                </li>
                <li>
                  <button v-if="r.status === EMasterStatus.Pending || r.status === EMasterStatus.Active" @click.prevent="reject(r.id)" class="w-full text-right block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">رد درخواست</button>
                </li>
              </ul>
            </div>
          </td>
...
import {
  ...
  RejectMasterRequest,
  GetMastersByAdmin
} from "~/services/admin/teachers.admin.service";
import {EMasterStatus, MasterDto, MasterFilterParams} from "~/models/master/MasterDto";
...
const loadData = async ()=>{
  const result = await GetMastersByAdmin(filterParams);
  if(result.isSuccess){
