  ...
  <u-table ref="dataTable">
    ...
    <template v-slot:table-body="{showOption}">
      <tr class="border-b dark:border-gray-700" v-for="(r,i) in requests">
        ...
        <td class="px-4 py-3">
            <u-badge v-if="r.status == 0" color="primary">در حال بررسی</u-badge>
            <u-badge v-if="r.status == 1" color="success"> فعال </u-badge>
            <u-badge v-if="r.status == 2" color="danger">{غیر فعال</u-badge>
        </td>
        <td class="px-4 py-3 flex items-center justify-end">
          ...
          <div class="table-option absolute hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
            <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="apple-imac-27-dropdown-button">
              <li>
                <button class="w-full text-right block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">غیر فعال کردن</button>
              </li>
            </ul>
          </div>
        </td>
