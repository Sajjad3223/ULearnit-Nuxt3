<template>
  <div>
    <u-divider title="مدرسین"/>

    <u-table >
      <template #table-header>
        <th scope="col" class="px-4 py-3">آواتار مدرس</th>
        <th scope="col" class="px-4 py-3">نام مدرس</th>
        <th scope="col" class="px-4 py-3">فایل رزومه دوره</th>
        <th scope="col" class="px-4 py-3">
          <span class="sr-only">عملیات</span>
        </th>
      </template>
      <template v-slot:table-body="{showOption}">
        <tr class="border-b dark:border-gray-700" v-for="(c,i) in 10">
          <th class="p-2">
            <img src="/imgs/user.png" class="rounded-full">
          </th>
          <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
            <NuxtLink to="/profile/1" class="link" >سجاد میرشبی</NuxtLink>
          </th>
          <td class="px-4 py-3">
            <a download href="#">
              <u-badge color="primary">فایل رزومه</u-badge>
            </a>
          </td>
          <td class="px-4 py-3 flex items-center justify-end">
            <button @click.prevent="showOption(i)" class="inline-flex items-center p-0.5 text-sm font-medium text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
              <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
              </svg>
            </button>
            <div ref="options" class="table-option absolute hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
              <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="apple-imac-27-dropdown-button">
                <li>
                  <NuxtLink to="/" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">قبول درخواست</NuxtLink>
                </li>
                <li>
                  <NuxtLink to="/" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">رد درخواست</NuxtLink>
                </li>
              </ul>
            </div>
          </td>
        </tr>
    </template>
    </u-table>
  </div>
</template>

<script setup lang="ts">
definePageMeta({
  layout:'admin',
})
</script>