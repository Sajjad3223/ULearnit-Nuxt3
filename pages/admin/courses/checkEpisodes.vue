    ...
    <div class="w-full mt-4">
      <u-table ref="dataTable" :pagination-data="paginationData">
        ...
        <template #table-header>
          <th scope="col" class="px-4 py-3">عنوان قسمت</th>
          <th scope="col" class="px-4 py-3">اسلاگ</th>
          <th scope="col" class="px-4 py-3">توضیحات</th>
          <th scope="col" class="px-4 py-3">رایگان؟</th>
          <th scope="col" class="px-4 py-3">زمان</th>
          <th scope="col" class="px-4 py-3">ویدئو</th>
          <th scope="col" class="px-4 py-3">فایل ضمیمه</th>
        ...
        <template v-slot:table-body="{showOption,hideAll}">
          <tr class="border-b dark:border-gray-700" v-for="(e,i) in episodes">
            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
              {{e.title}}
            </th>
            <td class="px-4 py-3">{{ e.englishTitle }}</td>
            <td class="px-4 py-3">{{ e.description }}</td>

            <td class="px-4 py-3">
              <u-badge v-if="e.isFree">رایگان</u-badge>
              <u-badge v-else color="dark">نقدی</u-badge>
            </td>
            <td class="px-4 py-3">{{e.time}}</td>
            <td class="px-4 py-3">{{e.token}}</td>
            <td class="px-4 py-3">
              <u-badge v-if="e.hasAttachment">دارید</u-badge>
              <u-badge v-else color="dark">ندارد</u-badge>
            </td>

            <td class="px-4 py-3">
              <u-badge v-if="e.episodeStatus === EEpisodeStatus.Pending" color="warning">در انتظار بررسی</u-badge>
              <u-badge v-if="e.episodeStatus === EEpisodeStatus.Checked" color="success">بررسی شده</u-badge>
              <u-badge v-if="e.episodeStatus === EEpisodeStatus.Rejected" color="danger">رد شده</u-badge>
            </td>
            <td class="px-4 py-3">{{new Date(e.creationDate).toLocaleDateString('fa-IR')}}</td>
            <td class="px-4 py-3 flex items-center justify-end">
              ...
              <div class="hidden table-option z-20 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                ...
                <li>
                    <button @click="publishEpisode(e.courseId,e.id)" class="w-full text-right block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">انتشار</button>
                  </li>
                </ul>
              </div>
            </td>
...
import {GetPendingEpisodes, PublishEpisode} from "~/services/admin/courses.admin.service";
...
const publishEpisode = async (courseId:number,episodeId:number)=>{
  const result = await PublishEpisode(courseId,episodeId);
  if(result.isSuccess)
  {
    successAlert();
    await loadData();
