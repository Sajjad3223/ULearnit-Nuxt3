<template>
  <div>
    ...
    <u-divider title="مدیریت کویز ها" />
    <div class="w-full mt-4">
      <u-table>
        ...
        <template #table-header>
          ...
          <th scope="col" class="px-4 py-3">عنوان سرفصل</th>
          <th scope="col" class="px-4 py-3">تعداد سوالات</th>
          <th scope="col" class="px-4 py-3">نمره کل</th>
          <th scope="col" class="px-4 py-3">نمره قبولی</th>
          <th scope="col" class="px-4 py-3">
            <span class="sr-only">عملیات</span>
          </th>
        </template>
        <template v-slot:table-body="{showOption,hideAll}">
          <tr class="border-b dark:border-gray-700" v-for="(q,i) in quizzes">
            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
              <NuxtLink :to="`/masterpanel/quizzes/${q.id}`" class="link" >{{ q.title }}</NuxtLink>
            </th>
            <td class="px-4 py-3">{{ q.sectionTitle }}</td>
            <td class="px-4 py-3">{{ q.totalQuestions }}</td>
            <td class="px-4 py-3">{{ q.totalScore }}</td>
            <td class="px-4 py-3">{{ q.passScore }}</td>
            <td class="px-4 py-3 flex items-center justify-end">
              ...
              <div class="hidden table-option z-20 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" >
                  <li>
                    <NuxtLink :to="`/masterpanel/quizzes/${q.id}`" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">جزئیات کویز</NuxtLink>
                  </li>
                  <li>
                    <NuxtLink :to="`/masterpanel/quizzes/edit/${q.id}`" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">ویرایش کویز</NuxtLink>
                  </li>
                  <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" >
                    <li>
                      <button @click="deleteQuiz(b.id)" class="w-full text-right block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">حذف مقاله</button>
                    </li>
                  </ul>
                </ul>
              </div>
...
import {QuizFilterData, QuizFilterParams} from "~/models/quiz/quizFilterData";
import {GetQuizzesOfMaster} from "~/services/quiz.service";
...
const quizzes = ref<QuizFilterData[]>();
const paginationData = ref<PaginationData>();

const route = useRoute();
const filterParams:QuizFilterParams = reactive({
...
onMounted(async ()=>{
  const result = await GetQuizzesOfMaster(filterParams);
  if(result.isSuccess){
    quizzes.value = result.data.data;
