          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div>
              <label for="category" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">دسته
                بندی</label>
              <select ref="categorySelect" id="category" v-model="editPostData.categoryId"
            ...
            <base-file-uploader upload-url="/roadmap/upload-banner" title="بنر مقاله" v-model="editPostData.imageName" accept-format="image"/>
            <base-file-uploader upload-url="/roadmap/upload-video" title="ویدئوی مقاله" v-model="editPostData.videoName" accept-format="video"/>
...
const editPostData:EditRoadmapPostViewModel = reactive({
  id: 0,
  title: "",
  slug: "",
  content: "",
...
const UpdatePost= async ()=>{
  const result = await EditRoadmapPost(editPostData);
  if(result.isSuccess)
  {
    successAlert();
