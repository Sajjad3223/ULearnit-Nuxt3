<template>
  <div id="data-table" class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden mt-4">
      ...
      <div class="w-full md:w-1/2">
        <form @submit.prevent="search" class="flex items-center">
          ...
          <div class="relative w-full">
            ...
            <input type="text" id="simple-search" v-model="searchInput"
    ...
    <nav v-if="hasFooter && paginationData !== undefined" class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    نمایش
                    <span class="font-semibold text-gray-900 dark:text-white">{{paginationData.take * paginationData.currentPage - paginationData.take + 1}}-{{paginationData.entityCount > paginationData.take ? paginationData.take * paginationData.currentPage : paginationData.entityCount}}</span>
                    از
                    <span class="font-semibold text-gray-900 dark:text-white">{{ paginationData.entityCount }}</span>
                </span>
      <ul class="inline-flex items-stretch -space-x-px">
        <li v-if="paginationData.currentPage > 1">
          <a href="javascript:void(0)" @click="changePageId(paginationData.currentPage - 1)"
        ...
        <li v-for="i in paginationData.pageCount" :key="i">
          <a href="javascript:void(0)" @click="changePageId(i)" v-if="i !== paginationData.currentPage"
          ...
          <a href="javascript:void(0)" v-else @click="changePageId(i)"
             class="flex items-center justify-center text-sm z-10 py-2 px-3 leading-tight text-primary-600 bg-primary-50 border border-primary-300 hover:bg-primary-100 hover:text-primary-700 dark:border-gray-700 dark:bg-gray-700 dark:text-white">{{i}}</a>
        </li>
        <li v-if="paginationData.currentPage < paginationData.endPage">
          <a href="javascript:void(0)" @click="changePageId(paginationData.currentPage + 1)"
...
const search = ()=>{
  ...
  router.push({
    path: currentPath,
    query: { ...queryParams, q:searchInput.value },
  });
}
