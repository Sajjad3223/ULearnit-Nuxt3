    ...
    <article :class="['mr-2 p-4 rounded-xl flex-1 text-base bg-white dark:bg-gray-800',{'dark:bg-amber-800':answer.isCorrectAnswer}]">
      ...
      <div class="flex items-center mt-4 space-x-4 space-x-reverse">
        <div class="inline-flex space-x-2 space-x-reverse">
          <button class="opacity-75 hover:opacity-100 transition-opacity" @click.prevent="deleteReaction" v-if="answer.userReaction && answer.userReaction === 'Like' ">
        ...
        <div class="inline-flex space-x-2 space-x-reverse">
          <button class="opacity-75 hover:opacity-100 transition-opacity" @click.prevent="deleteReaction" v-if="answer.userReaction && answer.userReaction === 'Dislike'">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" >
          ...
          <span>{{answer.dislikesCount}}</span>
        </div>
      </div>
    </article>
  </div>
...
<script setup lang="ts">
  ...
  import {DeleteReaction,ReactToQuestion, SetQuestionSolved} from "~/services/question.service";
  ...
  const emits = defineEmits(['questionSolved','reacted']);
  ...
  const deleteReaction = async ()=>{
    const result = await DeleteReaction(props.answer.id);
    if(result.isSuccess) {
      successAlert();
      emits('reacted');
    }
